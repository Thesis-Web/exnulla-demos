...
**Integration model:** *Model A — Static Artifact Embedding* (see `demo-integration-engineering-spec-exnulla-site-1-0-0.md`)  
...
- **Predicate attestations**: protocol verifies *properties* (predicates), not identity material.
...
Must integrate via `exnulla-site` **demo manifest + sync** pipeline:
- `exnulla-site/demos/manifest.json` pins commit SHA
...
- “This demo simulates predicate attestations: the protocol verifies *properties*, not identity.”
...
**Epoch secret**
\[
S_{v,e} = H(v \parallel ":" \parallel e)
\]

**Commitment**
\[
c_e(v,h) = H(S_{v,e} \parallel ":" \parallel m(h))
\]
...
\[
t_{a\to b}(c_a) = H("bridge:" \parallel e \parallel ":" \parallel c_a)
\]
...
**Note:** Because commitments are derived from `S_{v,e}`, `t === bCommitment` will generally *not* hold unless you explicitly model “resolver accepted mapping”.  
...
- When bridge enabled, treat “equivalence under resolver” as:
  ...
  - Display “Resolver token” and show that *protocol* can accept the resolver token without learning identity.
...
```ts
export type AuditEvent = { ts: number; code: string; message: string; data?: Record<string, unknown> };
...
### 13.1 Add to `exnulla-site/demos/manifest.json`
...
8. Pin commit SHA in `exnulla-site/demos/manifest.json` and enable.
9. Run `exnulla-site` CI locally (or via PR) to validate pipeline.
